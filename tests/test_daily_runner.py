...
class DailyRunnerTests(unittest.TestCase):
 ...
 def test_manual_runs_all_phases_without_scheduler(self):
  stages,seen=self.stages();result=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
 ...
 def test_scheduled_uses_same_runner_and_requires_admission(self):
  ...
  with self.assertRaisesRegex(DailyError,'scheduler admission'):run_daily(profile=self.profile('scheduled'),capability_schema=self.schema,entrypoint='scheduled',operation_id='daily-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
  self.assertEqual('scheduled',run_daily(profile=self.profile('scheduled'),capability_schema=self.schema,entrypoint='scheduled',operation_id='daily-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),scheduler_admission=lambda:True).entrypoint)
 def test_runner_rejects_unverified_or_missing_phase(self):
  stages,_=self.stages();stages['catalog']=lambda _:{'verified':False}
  with self.assertRaisesRegex(DailyError,'not verified: catalog'):run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
  del stages['catalog']
  with self.assertRaisesRegex(DailyError,'missing required daily phase: catalog'):run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
 def test_planned_pause_and_resumption_use_durable_predecessor_output(self):
  stages,seen=self.stages(); checkpoints=[]
  paused=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),stop_after='catalog',checkpoint=lambda phase,result,outcome: checkpoints.append((phase,result)) or f'checkpoint-{phase}')
  ...
  resumed=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-001',attempt_id='attempt-002',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='catalog',durable_predecessor_output=checkpoints[-1][1])
 ...
 def test_stale_auth_capacity_and_resumed_non_progress_block(self):
  ...
  with self.assertRaisesRegex(CapabilityError,'authentication'):qualify_execution(stale,self.schema,operation='daily-run',entrypoint='manual',required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
  plan=qualify_execution(self.profile('manual'),self.schema,operation='daily-run',entrypoint='manual',required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),requested_records=99,requested_bytes=999999)
  ...
  with self.assertRaisesRegex(DailyError,'made no progress'):run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-001',attempt_id='attempt-002',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='catalog',durable_predecessor_output={'verified':True,'phase':'catalog'},require_progress_after_resume=True)
 def test_no_new_message_run_still_regenerates_required_outputs(self):
  ...
  result=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-empty-001',attempt_id='attempt-001',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
 ...
 def test_measured_budget_stops_before_next_phase_and_resumes(self):
  ...
  paused=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-budget',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),max_elapsed_ms=4,estimated_phase_ms={'preflight':1,'discover':1,'catalog':4},monotonic_clock=lambda:0.0,checkpoint=lambda phase,result,outcome:checkpoints.append((phase,result,outcome)) or phase)
  ...
  resumed=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-budget',attempt_id='attempt-2',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='discover',durable_predecessor_output=checkpoints[-1][1])
 ...
 def test_budget_counts_admission_and_checkpoint_elapsed(self):
  ...
  with self.assertRaisesRegex(DailyError,'cannot complete the next minimum unit'):
   run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-budget',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),max_elapsed_ms=5,estimated_phase_ms={'preflight':1},monotonic_clock=lambda:next(admission_times))
  ...
  paused=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-budget',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),max_elapsed_ms=6,estimated_phase_ms={'preflight':1,'discover':2},reserve_ms=1,monotonic_clock=lambda:clock[0],checkpoint=checkpoint)
 ...
 def test_budget_validation_rejects_unknown_negative_and_bad_numeric_types(self):
  stages,_=self.stages()
  base=dict(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-budget',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),monotonic_clock=lambda:0.0)
 ...
 def test_budget_and_planned_boundaries_require_durable_checkpoint(self):
  stages,_=self.stages();base=dict(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-budget',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'))
 ...
 def test_resumed_budget_preserves_history_without_replaying_predecessors(self):
  stages,seen=self.stages();checkpoints=[]
  paused=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-resume',attempt_id='attempt-2',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='discover',durable_predecessor_output={'verified':True,'phase':'discover'},max_elapsed_ms=3,estimated_phase_ms={'catalog':1,'reconcile':3},monotonic_clock=lambda:0.0,checkpoint=lambda phase,result,outcome:checkpoints.append((phase,result,outcome)) or f'checkpoint-{phase}-{outcome}')
  self.assertEqual(('preflight','discover','catalog'),paused.completed_phases);self.assertEqual(['catalog'],[phase for phase,_ in seen])
  resumed=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-resume',attempt_id='attempt-3',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='catalog',durable_predecessor_output=checkpoints[-1][1])
  ...
  with self.assertRaisesRegex(DailyError,'verified durable predecessor output'):
   run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-resume',attempt_id='attempt-3',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='catalog',durable_predecessor_output={'verified':False})
 def test_partial_phase_checkpoints_and_resumes_inside_same_phase(self):
  calls=[];clock=[0.0];checkpoints=[]
  stages,_=self.stages()
  def catalog(previous):
   calls.append(tuple(previous.get('completed_units',())))
   if not previous.get('completed_units'):
    clock[0]=0.005
    return {'verified':True,'phase_complete':False,'completed_units':['message-1'],'remaining_work':{'next':'message-2'}}
   return {'verified':True,'phase_complete':True,'completed_units':['message-1','message-2'],'remaining_work':{}}
  stages['catalog']=catalog
  estimates={phase:0 for phase in PHASES};estimates['catalog']=1
  paused=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-units',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),max_elapsed_ms=5,estimated_phase_ms=estimates,monotonic_clock=lambda:clock[0],checkpoint=lambda phase,result,outcome:checkpoints.append((phase,result,outcome)) or f'{phase}-{outcome}')
  self.assertEqual('NEEDS_CONTINUATION',paused.outcome);self.assertEqual('catalog',paused.current_phase);self.assertEqual(('preflight','discover'),paused.completed_phases)
  clock[0]=0.0
  resumed=run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-units',attempt_id='attempt-2',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),resume_after='catalog',durable_predecessor_output=checkpoints[-1][1],require_progress_after_resume=True)
  self.assertEqual('COMPLETE',resumed.outcome);self.assertEqual([(),('message-1',)],calls);self.assertEqual(PHASES,resumed.completed_phases)
 def test_partial_phase_cannot_advance_without_real_durable_checkpoint_identity(self):
  for checkpoint in (None,lambda _phase,_result,_outcome:None,lambda _phase,_result,_outcome:' '):
   with self.subTest(checkpoint=checkpoint):
    stages,_=self.stages();calls=[]
    def catalog(previous):
     calls.append(tuple(previous.get('completed_units',())))
     if not previous.get('completed_units'):
      return {'verified':True,'phase_complete':False,'completed_units':['u1'],'remaining_work':{'next':'u2'}}
     return {'verified':True,'phase_complete':True,'completed_units':['u1','u2'],'remaining_work':{}}
    stages['catalog']=catalog
    with self.assertRaisesRegex(DailyError,'durable checkpoint'):
     run_daily(profile=self.profile('manual'),capability_schema=self.schema,entrypoint='manual',operation_id='daily-units',attempt_id='attempt-1',stages=stages,required_capabilities=('storage.read_complete','mail.search','tasks.list_complete'),checkpoint=checkpoint)
