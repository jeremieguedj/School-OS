...
operations, contracts, adapters and fictional examples are authored. The
[second-round remediation](ROUND-2-REMEDIATION.md) is implemented in source and
its deterministic local checks pass. Limited first-round setup/ingestion/query
trials exposed material failures; the product is not qualified for routine use.
...
its required saved-route audit again; Work passed setup, saved partial ingestion
and produced answers with a report artifact. Work's saved tasks contain semantic
defects, including an omitted response deadline. Its seven questions graded five
pass, one qualified and one fail. Keep all earlier attempts and private evidence
