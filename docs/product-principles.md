...
No single metadata field or field combination is guaranteed unique or universally available. Missing fields remain unknown, and connector-generated filenames or incomparable inventories must not become identity requirements. Optional provider/RFC identifiers may accelerate retrieval but must not decide identity or acceptance. A unique metadata association among observed candidates is not proof of a unique physical delivery or equal content. Preserve known distinct occurrences and visible collisions without silently merging them. An undisclosed occurrence with identical metadata cannot always be detected under this policy; do not promise zero identity errors. An unresolved lookup must not erase prior knowledge or stop unrelated work.
