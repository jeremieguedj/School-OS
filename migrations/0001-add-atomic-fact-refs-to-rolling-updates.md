...
3. Require the fact's local received date to equal the bullet's enclosing received-day heading. For a legacy record that does not store a per-Fact local date, use its record-level date only when the header contains exactly one unambiguous local calendar date; a date range, multiple dates, or no date is a blocking exception. Derive the fact's presentation group through the private configured group-projection rule, then require that derived group to equal the bullet's displayed group. Do not compare raw entity-scope text directly to the displayed group.
...
- every referenced Fact passes the update/guideline/action, local-date (including only the permitted singular-record-date fallback), and configured-group-projection filters above;
