...
PREVIEW_REQUIRED_CAPABILITIES = tuple(
    capability for capability in REQUIRED_CAPABILITIES if capability != "mail.send"
)
...
def _restore_daily_state(
    *, manager: _CheckpointManager, store: ArtifactStore,
    ...
    if delivered is not None:
        if delivered.get("delivery_reserved") is False:
            context.update({
...
class ConnectedDailyRuntime:
    ...
    def run(self, *, entrypoint: str, operation_id: str, attempt_id: str,
            scheduler_admitted: bool = False, delivery_variant: str | None = None,
            preview_only: bool = False) -> OperationResult:
        if preview_only and entrypoint != "manual":
            raise ConnectedDailyError("the unsent preview is manual-only")
        if preview_only and delivery_variant is not None:
            raise ConnectedDailyError("the unsent preview cannot select a delivery variant")
        ...
        manager = _CheckpointManager(
            ...
            scope={
                ...
                "operation_mode": "unsent_preview" if preview_only else "delivery",
        ...
        def brief_delivery(_previous: Mapping[str, Any]) -> dict[str, Any]:
            ...
            context.update({"brief_input": brief_input, "brief_html": brief_html, "brief_text": brief_text})
            if preview_only:
                context["preview_rendered"] = True
                return {
                    "verified": True, "phase_complete": True,
                    "brief_input": _artifact_mapping(brief_input),
                    "brief_html": _artifact_mapping(brief_html),
                    "brief_text": _artifact_mapping(brief_text),
        ...
        def commit(_previous: Mapping[str, Any]) -> dict[str, Any]:
            ...
            if catalog_result is None or catalog_result.proposed_cursor is None:
                raise ConnectedDailyError("commit requires a complete eligible catalog cursor")
            if preview_only:
                if context.get("preview_rendered") is not True or delivery is not None:
                    raise ConnectedDailyError("preview commit requires verified unsent render artifacts")
            elif delivery is None or delivery.get("outcome") not in {"confirmed", "reconciled", "suppressed"}:
                raise ConnectedDailyError("delivery commit requires exact delivery")
            evidence = canonical_json_bytes({
                "schema_version": 1, "operation_id": operation_id, "attempt_id": attempt_id,
                "outcome": "PREVIEW_READY" if preview_only else "COMPLETE",
                "delivery_reserved": not preview_only,
                "catalog_index": _reference_mapping(mutable["source_catalog_index"]),
                "canonical_tasks": _reference_mapping(mutable["canonical_tasks"]),
                "provider_state": _reference_mapping(mutable["task_sync_state"]),
                "delivery": None if preview_only else {key: value for key, value in delivery.items() if key != "ledger"},
                "brief_input": _artifact_mapping(context["brief_input"]),
                "brief_html": _artifact_mapping(context["brief_html"]),
                "brief_text": _artifact_mapping(context["brief_text"]),
        ...
        try:
            result = run_daily(
                ...
                stages=stages,
                required_capabilities=PREVIEW_REQUIRED_CAPABILITIES if preview_only else REQUIRED_CAPABILITIES,
            ...
            raise
        if result.outcome == "COMPLETE" and preview_only:
            result = OperationResult(
                "PREVIEW_READY", result.operation_id, result.attempt_id,
                result.entrypoint, result.current_phase, result.checkpoint_reference,
                "source/task cursor commit and deterministic unsent brief verified",
                result.completed_phases, result.outputs,
            )
        if result.outcome in {"COMPLETE", "PREVIEW_READY"}:
