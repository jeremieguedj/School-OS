...
For an ordinary daily brief, first validate that all intended emails and content
in the declared scope have been discovered, processed, saved and verified. An
agent's internal batch boundary does not satisfy this gate. If relevant
ingestion or coverage is blocked or unverifiable, report the scoped blocker and
do not issue the ordinary completed daily brief.

A manual partial or provisional brief remains a pending policy choice. Do not
use a recipe to treat one as approved, or use the qualified-query rule as
authority for a partial email delivery. Other kinds of briefs must still state
their actual coverage truthfully and must not imply a broader completed read.
...
Use the authorized route and recipient for this run. Inspect the actual result
through available authorized evidence. A generic success response does not
prove delivery, and an unknown outcome must be reconciled before any repeat.
...
The retention period and whether audio is prepared before or after another
delivery remain pending choices, so a recipe must not silently decide them.
Unsupported or failed audio does not become a successful output, and the recipe
