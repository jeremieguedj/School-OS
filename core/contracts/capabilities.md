...
Onboarding writes an observed capability profile to the private instance state. It records no secrets and no private source content. The conformance unit is the exact runtime, provider, authorization, and execution surface. A `manual` profile may qualify a direct attended operation with storage, mail, and task evidence and no scheduler. A `scheduled` profile additionally requires the selected scheduler, its network path, and observed scheduler behavior. Manual evidence cannot authorize scheduled execution.
